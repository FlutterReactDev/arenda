 ...
 */
export const get2GISEventName = <T>(propName: string): keyof T =>
  propName.substring(2).toLowerCase() as keyof T;

/**
 * Принимает строку (имя пропса).
 * Возвращает true, если строка начинается с "on" (считаем что это обработчик события)
 */
export const isReactHandlerKey = (propName: string) =>
 ...
 */
export const defer = <T extends (...args: any) => any>(
  func: T,
  wait?: number
) => {
  let timerId: number;
