...
export interface Clusterer2GISProps
  extends ClustererOptionsOverride,
    Partial<ClustererEventHandlerTable> {
...
const Clusterer2GISComponent: FC<Clusterer2GISProps> = (props) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < inputWebglMarkers.length; i++) {
      ...
      const markerStyleCfg = webglMarkerStyle?.(
        inputMarker.coordinates,
        inputMarker.userData
      );
  ...
  const clusterStyleFn = useCallback<ClusterStyleFn>(
    (pointsCount, target) => {
      const clusterStyleCfg =
        clusterStyleRef.current ||
        clusterStyleFnRef.current?.(pointsCount, target);
