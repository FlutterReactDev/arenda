...
export interface Marker2GISProps
  extends MarkerOptions,
    Partial<DynamicObjectEventHandlerTable> {}
...
const Marker2GISComponent: FC<Marker2GISProps> = (props) => {
  ...
  useEffect(() => {
    markerOptionsRef.current = getOptions<
      MarkerOptions,
      Marker2GISProps,
      string
    >(props, isOptionsKey);
  ...
  const forceReCreateInstance =
    useReCreateInstanceWithSetterlessProps<MarkerOptions>(
  ...
  useEffect(() => {
    ...
    const instance = new mapGLBundle.Marker(
      mapInstance,
      markerOptionsRef.current
    );
