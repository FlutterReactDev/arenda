...
import { thirdFormSchema } from "../model/schemas/thirdFormSchema";
...
interface ThirdFormStepProps {
  onChangeScreen: (index: number) => void;
}
export const ThirdFormStep: FC<ThirdFormStepProps> = (props) => {
  const { onChangeScreen } = props;
  const dispatch = useAppDispatch();
  const thirdForm = useAppSelector(getForm(3, 0)) as Yup.InferType<
    typeof thirdFormSchema
  >;
  ...
  const {
    ...
    trigger,
    setValue,
  } = useForm<Yup.InferType<typeof thirdFormSchema>>({
  ...
  const onSubmit = (data: Yup.InferType<typeof thirdFormSchema>) => {
    onChangeScreen(4);
    dispatch(
      ...
      <Stack spacing={2}>
        ...
        <FormCard>
          <HStack justifyContent={"space-between"}>
            <Button colorScheme="gray" onClick={onPrev} variant={"outline"}>
              Назад
            </Button>
